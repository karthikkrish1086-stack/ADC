...
def render_code(body_lines):
    ...
    for raw in body_lines:
        for line in raw.split("\n"):
            ...
            # split off trailing comment for muted styling
            if "  #" in esc:
...
if run:
    ...
    with right:
        ...
        st.markdown(f'<div class="codehead" style="border:1px solid var(--line);'
                    ...
                    f'<span class="{"pass" if ok else ""}">'
                    f'{"✓ PARITY PASS" if ok else "✗ MISMATCH"}</span></div>',
                    unsafe_allow_html=True)
    ...
    st.info("Press **Run conversion** to parse the workflow, generate Python, execute it on the data above, and validate parity.", icon="▶")
